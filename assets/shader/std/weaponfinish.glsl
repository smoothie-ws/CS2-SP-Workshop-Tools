...
void applyFinish(V2F inputs, out ShaderOutputs outputs) {
    ...
    vec2 baseRough = tex2D(uBaseRough, uv).rg;
    ...
    : linear2sRGB(baseAO);
    // roughness
    outputs.orm.g = mix(patternRough + (1.0 - dirt) * dirtMult * uWearAmt, baseRough.r, outputs.wear);
    // metallic
    outputs.orm.b = mix(patternMetal * mix(1.0, dirt * 0.75 + 0.25, uWearAmt), baseRough.g, outputs.wear);
}
