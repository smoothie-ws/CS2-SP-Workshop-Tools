...
class UI:
    ...
    def start(self, status:int, callback, error_callback):
        if status  == QtQuick.QQuickView.Ready:
            ...
            help_action.triggered.connect(self.internal.on_help)
            menu.addSeparator()
            clear_docs_action = menu.addAction("Clear documents")
            clear_docs_action.triggered.connect(self.internal.on_clear_docs)
            sp.ui.add_menu(menu)
