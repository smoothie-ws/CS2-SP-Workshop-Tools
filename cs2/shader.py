...
def preprocess(code: str, macros: dict={}):
    ...
    def current_condition():
        return True if current_level() == 0 else condition_stack[-1]
    ...
    for line in code.split("\n"):
        if not line.startswith("//:"):
            ...
            #undef
                ...
                if current_level() > 0:
                    condition_stack[-1] = not current_condition()
                continue

            #elif
            elif stripped_line.startswith("#elif"):
                if current_level() > 0:
                    condition_stack[-1] = not current_condition() and eval_expr(stripped_line[5:].strip())
                continue
            ...
            if not skipping() and stripped_line != "":
                processed_lines.append(line)
        else:
            processed_lines.append(line)

    assert not current_level() > 0, "Missing #endif"
    assert not current_level() < 0, "Too many #endif"
